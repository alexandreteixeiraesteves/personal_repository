...
ssize_t gistre_card_read(struct file *file, char __user *buf,
	...
	if (dev->last_cmd == NONE){
		pr_info("[read_error] You need to use a command before !");
		return 0;
	}

	if (dev->last_cmd != MEM_READ) {
		pr_info("[read_error] You need to use mem_read before.\n");
		return 0;
	}

	ret = regmap_write(dev->map, MFRC522_CMDREG, MFRC522_IDLE);
	if (ret < 0) {
		pr_info("[read_error] : Error when the variable CMD reseted.");
		return -EFAULT;
	}
	...
	ret = regmap_write(dev->map,
			...
			MFRC522_FIFOLEVELREG_FLUSH);
	if (ret < 0) {
		pr_info("[read_error] : Error when fifo flushed");
		return -EFAULT;
	}

	ret = regmap_write(dev->map, MFRC522_CMDREG, MFRC522_MEM);
	if (ret < 0) {
		pr_info("[read_error] : Error command MEM_READ in using the map.");
		return -EFAULT;
	}

	//size_t len_content = dev->c_len;
	size_t len_content = GISTRE_CARD_BUFSIZE; //TODO modified  
	...
	dev->last_cmd = NONE;
	return len; //TODO maybe return GISTRE_CARD_BUFSIZE
}
